...
// from: https://eklausmeier.wordpress.com/2017/04/24/converting-wordpress-export-file-to-hugo/
// modified quickly and badly by James Shubin <james@shubin.ca>
	...
	for _, v := range delimList {
		...
		for {
			...
			if tx2 := strings.Index(s[tx0+tx1+lenvstart:], v.stop); tx2 > 0 {
				...
				// remove <strong> & </strong> tags inside code blocks :(
				// TODO: this doesn't work, it takes out other ones elsewhere too!
				//t = strings.Replace(t, "<strong>", "", -1)
				//t = strings.Replace(t, "</strong>", "", -1)
...
var replaceList = []struct {
	...
	// backup for unlabelled code blocks
	{regexp.MustCompile("\n\\[code\\]\\s*\n"), "\n{{< highlight >}}\n"},
	// change [code gutter="false"] too
	// TODO: search for `gutter="false"` getting added to your source after
	{regexp.MustCompile("\n\\[code gutter=\\\"false\\\"\\]\\s*\n"), "\n{{< highlight >}}\n"},
...
func hugofy(bodyStr string) string {
	...
	s = strings.Replace(s, "<a href=\""+config["baseURL"], "<a href=\"/blog", -1)
	s = strings.Replace(s, "<a href=\""+config["baseURLalt"], "<a href=\"/blog/", -1)
	...
	// replace [caption] and [/caption] with an html table
	new_s := []string{}
	lines := strings.Split(s, "\n")
	for _, line := range lines {
		if strings.HasPrefix(line, "[caption") && strings.HasSuffix(line, "[/caption]") {
			line = line[strings.Index(line, "]")+len("]") : strings.Index(line, "[/caption]")]
			// there's usually an </a> in the middle where we want to split
			a := line[:strings.Index(line, "</a>")+len("</a>")]
			b := line[strings.Index(line, "</a>")+len("</a>"):]
			line = (`<table style="text-align:center; width:80%; margin:0 auto;"><tr><td>` + a + "</td></tr><tr><td>" + b + "</td></tr></table></br />")
		}
		new_s = append(new_s, line)
	}

	return strings.Join(new_s, "\n")
}
...
func commaSep(list map[int]string, prefix string) string {
	...
	for _, x := range keys { // sorted, deterministic
		lineLen += copy(line[lineLen:], sep)
		lineLen += copy(line[lineLen:], "\"")
		lineLen += copy(line[lineLen:], x)
		lineLen += copy(line[lineLen:], "\"")
		sep = ", "
	}
	lineLen += copy(line[lineLen:], "]\n")
...
func wrtPostFile(frontmatter map[string]string, cats, tags map[int]string, body []byte, bodyLen int) {
	...
	original_fname := filepath.Join(dirname, fname)
	...
	// make a dir for the images per post
	//base := original_fname[strings.LastIndex(original_fname, "/")+len("/"):]
	if err := os.Mkdir(original_fname, 0770); err != nil {
	...
	originalURL := ""
	if original_fname != "" {
		original_fname = strings.TrimPrefix(original_fname, "content/post/")
		original_fname = fmt.Sprintf("https://ttboj.wordpress.com/%s/", original_fname)
		originalURL = "original_url = \"" + original_fname + "\"\n"
	}
	mathLine := ""
		...
		if inItem && !empty {
			if tx1 = strings.Index(s, "<title>"); tx1 >= 0 {
				...
				if tx2 = strings.Index(s, "</wp:attachment_url>"); tx2 >= tx1 {
					...
					orig_url := strings.TrimSuffix(frontmatter["link"], strings.ToLower(frontmatter["title"])+"/")

					skip_last := orig_url[0 : len(orig_url)-1] // don't look for last /
					base := orig_url[strings.LastIndex(skip_last, "/")+len("/"):]

					a.hugoFname = base + a.fname // e.g., post.../cisco1.png
					attachm[url] = a
